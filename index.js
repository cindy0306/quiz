...
let thanks = '<div class="thanks"><div class="thanks__tick">✔ </div><h1 class="thanks__title">빡공 화이팅👊</h1></div>';
...
let currentIndex = Number($('.quiz__step--current').attr('data-question'));
  ...
  $('.summary__item').remove();
    $('.quiz__step:not(.quiz__summary)').each(function(index, item){
      ...
      let realAnswer = ''
      if(index == 0) realAnswer = ' 👉🏻 정답 : 확대/축소'
      if(index == 1) realAnswer = ' 👉🏻 정답 : 실행취소'
      if(index == 2) realAnswer = ' 👉🏻 정답 : 다시실행'
      let node = '<div class="summary__item"><div class="question__emoji">'+icon+'</div>'+answer+realAnswer+'</div>'
      console.log("야호" ,node)
      $('#summary').append(node)
    })
  }
